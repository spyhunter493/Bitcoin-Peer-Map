...
/**
 * @param {{getNodeInfo: () => import('../types').NodeInfo | null; getCurrency: () => string; currencyMeta: Record<string, {symbol: string; decimals: number}>; formatBytes: (bytes: number) => string}} options
 */
function create(options) {
    ...
    function openNodeInfo() {
        ...
        const mempoolRequest = api.getJson(`/api/mempool?currency=${encodeURIComponent(currency)}`, { signal: dialog.signal });
        mempoolRequest
            .then((data) => {
