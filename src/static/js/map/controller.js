...
function create() {
    ...
    const tooltipEl = required('#node-tooltip');
        ...
        onAction(action) {
            switch (action.type) {
                case 'highlight':
                    ...
                    break;
                case 'pin':
                    dashboard.interaction.pinnedNode = mapView.nodes.find((node) => node.peerId === action.peerId && node.alive) || null;
                    break;
                case 'networks':
    ...
    function applyPeerSnapshot(peers) {
        ...
        for (const peer of peers) {
            ...
            let lat, lon;
            const isPrivate =
                peer.location_status === 'private' || peer.location_status === 'unavailable' || peer.location_status === 'pending';
        ...
        const selection = dashboard.interaction.groupSelection;
        if (selection) {
            const group = mapView.nodes.filter((node) => {
                if (!node.alive || (selection.privateGroup && !PRIVATE_NETS.has(node.peer.network))) return false;
            ...
            dashboard.interaction.mapFilterPeerIds = new Set(group.map((node) => node.peerId));
            const restore = BPMDomState.capture(tooltipEl);
            showGroupSelectionList(group, selection.mx, selection.my, selection.privateGroup);
            restore();
        }
 ...
 * @param {number} originY */
    ...
    function showGroupSelectionList(group, mx, my, privateGroup = false) {
        if (!dashboard.interaction.groupSelection) {
            const position = screenToWorld(mx, my);
            dashboard.interaction.groupSelection = {
                point: project(position.lon, position.lat),
                radiusX: 12 / (mapView.width * view.zoom),
                radiusY: 12 / (mapView.height * view.zoom),
                mx,
                my,
                privateGroup,
            };
        ...
        // Bind close button
        const closeBtn = query('.tt-group-close', tooltipEl);
        if (closeBtn) {
            closeBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                dashboard.interaction.pinnedNode = null;
                dashboard.interaction.groupedNodes = null;
                dashboard.interaction.groupSelection = null;
                dashboard.interaction.highlightedPeerId = null;
                hideTooltip();
        ...
        queryAll('.tt-group-clickable', tooltipEl).forEach((row) => {
            row.addEventListener('click', (e) => {
                ...
                if (node) {
                    ...
                    if (distribution) {
                        ...
                        if (peerData) {
                            distribution.openPeerDetailPanel(
                                peerData,
                                'map-group',
                                group.map((item) => item.peerId)
                            );
                        }
                    }
    ...
    function buildPeerDetailHtml(node, pinned, hasBackNav) {
        ...
        // Address display
        const addrDisplay = node.peer.ip && node.peer.port ? `${node.peer.ip}:${node.peer.port}` : node.peer.addr || '—';
        ...
        const locationStr =
            locationParts.length > 0 ? escapeHtml(locationParts.join(', ')) : '<span class="tt-muted">Private Network</span>';
    ...
    function showPinnedPeerDetail(node, mx, my, hasBackNav) {
        ...
        const dcBtn = query('.tt-disconnect', tooltipEl);
        if (dcBtn) {
        ...
        // Bind back/exit link
        const backLink = query('.tt-back-link', tooltipEl);
        if (backLink) {
 ...
 * @param {import('../types').MapNode} node */
    ...
    function frame(timestamp) {
        ...
        if (!privateState.privateNetMode) {
            if (dashboard.interaction.asLineGroups && dashboard.interaction.asLineGroups.length > 0) {
                ...
                dashboard.interaction.asLinePeerIds &&
                dashboard.interaction.asLinePeerIds.length > 0 &&
                dashboard.interaction.asLineColor
            ) {
        ...
        if (
            dashboard.interaction.highlightedPeerId !== null &&
            (!dashboard.interaction.pinnedNode || dashboard.interaction.highlightedPeerId !== dashboard.interaction.pinnedNode.peerId)
        ) {
        ...
        if (dragging && !dragMoved) {
            ...
            if (privateState.privateNetMode) {
                ...
                if (privateGroup.length > 1) {
                    ...
                    peerTable.renderPeerTable();
                    showGroupSelectionList(privateGroup, e.clientX, e.clientY, true);
            ...
            if (group.length > 0) {
                ...
                if (
                    group.every(function (n) {
                        return PRIVATE_NETS.has(n.peer.network);
                    })
                ) {
                ...
                if (group.length === 1) {
                    ...
                    if (
                        ...
                        dashboard.interaction.pinnedNode.peerId === node.peerId &&
                        distribution &&
                        distribution.isPeerDetailActive()
                    ) {
                        dashboard.interaction.pinnedNode = null;
                        dashboard.interaction.highlightedPeerId = null;
                        dashboard.interaction.hoveredNode = null;
    ...
    canvas.addEventListener(
        ...
        (e) => {
            ...
            const sxAfter =
                (pBefore.x - 0.5) * mapView.width * mapView.target.zoom + mapView.width / 2 - mapView.target.x * mapView.target.zoom;
            const syAfter =
                (pBefore.y - 0.5) * mapView.height * mapView.target.zoom + mapView.height / 2 - mapView.target.y * mapView.target.zoom;
