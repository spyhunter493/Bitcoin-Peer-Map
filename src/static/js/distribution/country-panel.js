...
/**
 * @param {import('../types').EntityPanelOptions & {peers: import('../types').Peer[]; providers: import('../types').SummaryProvider[]}} options
 */
function render(options) {
    ...
    if (fullGroup.connTypesList && fullGroup.connTypesList.length > 0) {
        ...
        html += view.row(
            'Inbound',
            seg.isOthers
                ? allPeers.filter(function (p) {
                      return p.direction === 'IN';
                  }).length
                : fullGroup.inboundCount
        );
        html += view.row(
            'Outbound',
            seg.isOthers
    ...
    if (providers.length > 0) {
        ...
        for (var pi = 0; pi < providers.length; pi++) {
            ...
            html += view.interactiveRow(
                prov.asNumber + ' \u00b7 ' + pName,
                prov.peerCount + ' peer' + (prov.peerCount !== 1 ? 's' : ''),
                prov.peerIds,
                'country-provider'
            );
        }
    ...
    if (fullGroup.versions && fullGroup.versions.length > 0) {
        ...
        for (var vi = 0; vi < fullGroup.versions.length; vi++) {
            html += view.interactiveRow(
                ...
                fullGroup.versions[vi].peers.map(function (p) {
                    return p.id;
                }),
                'software'
            );
    ...
    if (fullGroup.servicesCombos && fullGroup.servicesCombos.length > 0) {
        ...
        for (var si = 0; si < fullGroup.servicesCombos.length; si++) {
            html += view.interactiveRow(
                ...
                fullGroup.servicesCombos[si].peers.map(function (p) {
                    return p.id;
                }),
                'services'
            );
