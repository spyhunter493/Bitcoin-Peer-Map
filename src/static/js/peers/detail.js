...
function peerDetailRow(label, value, allowHtml) {
    ...
    return (
        '<div class="as-detail-sub-row"><span class="as-detail-sub-label">' +
        escapeHtml(label) +
        '</span><span class="as-detail-sub-val">' +
        renderedValue +
        '</span></div>'
    );
}
...
/**
 * @param {string | undefined} abbreviations
 * @param {Readonly<Record<string, import('../types').ServiceFlag>>} [serviceFlags]
 */
function renderServiceFlagList(abbreviations, serviceFlags) {
    ...
    for (const abbreviation of abbreviations.split(/\s+/)) {
        ...
        html +=
            '<div class="service-flag-row" title="' +
            escapeHtml(title) +
            '">' +
            '<span class="service-flag-abbr">' +
            escapeHtml(flag) +
            '</span>' +
            '<span class="service-flag-label">' +
            escapeHtml(label) +
            '</span>' +
            '<span class="service-flag-rpc">' +
            escapeHtml(rpc) +
            '</span></div>';
    }
    return html + '</div>';
 ...
 * @param {string | null} [providerColor]
 */
...
/**
 * @param {import('../types').Peer} peer
 * @param {import('../types').PeerDetailOptions} [options]
 */
function renderPeerDetails(peer, options) {
    ...
    const nowSeconds = config.nowSeconds == null ? Math.floor(Date.now() / 1000) : config.nowSeconds;
    const connectionTypes = config.connectionTypeLabels || {};
    const asName = presentation.asOrg || peer.asname || '';
    let html = '';
    html +=
        '<div class="peer-popup-badge" style="border-color:' +
        presentation.networkColor +
        ';color:' +
        presentation.networkColor +
        '">' +
        escapeHtml(presentation.networkLabel) +
        '</div>';
    html += '<div class="peer-popup-header">';
    if (config.showBack) {
        html += '<button type="button" class="peer-popup-back" ' + 'aria-label="Back to peers at this location">\u2190 List</button>';
    }
    html += '<div class="peer-popup-circle" style="background:' + presentation.networkColor + '"></div>';
    html +=
        '<div class="peer-popup-title"><div class="peer-popup-name" id="peer-popup-title" style="color:' +
    ...
    html += peerDetailRow(
        'Conn Type',
        (Object.hasOwn(connectionTypes, peer.connection_type) ? connectionTypes[peer.connection_type] : null) ||
    ...
    html += peerDetailRow('Last Recv', peer.lastrecv ? distributionData.fmtDuration(nowSeconds - peer.lastrecv) + ' ago' : '\u2014');
    html += peerDetailRow('Last Block', peer.last_block ? distributionData.fmtDuration(nowSeconds - peer.last_block) + ' ago' : '\u2014');
    html += peerDetailRow(
        'Last Tx',
    ...
    html += peerDetailRow('Services', renderServiceFlagList(peer.services_abbrev || '', config.serviceFlags), true);
    ...
    if (peer.transport_protocol_type) {
        html += peerDetailRow('Transport', peer.transport_protocol_type === 'v2' ? 'v2 (BIP324 encrypted)' : peer.transport_protocol_type);
    }
    if (peer.session_id) {
        html += peerDetailRow(
            'Session ID',
            '<span style="font-size:9px;word-break:break-all">' + escapeHtml(peer.session_id) + '</span>',
            true
        );
    }
    if (peer.minfeefilter != null) {
        html += peerDetailRow('Min Fee Filter', peer.minfeefilter > 0 ? (peer.minfeefilter * 100000000).toFixed(0) + ' sat/kvB' : 'None');
    }
    html += '</div>';
...
function renderPeerGroup(peers) {
    let html =
        '<div class="peer-popup-header" style="justify-content:center">' +
        '<div class="peer-popup-title" style="text-align:center">' +
        '<div class="peer-popup-name" id="peer-popup-title">' +
        peers.length +
        ' Peers at This Location</div>';
    if (peers[0]) {
        const location = [peers[0].city, peers[0].regionName, peers[0].country].filter(Boolean).join(', ');
        if (location) html += '<div class="peer-popup-addr">' + escapeHtml(location) + '</div>';
    }
    html +=
        '</div></div><div class="peer-popup-scroll"><div class="peer-popup-section">' +
        '<div class="peer-popup-section-title">Select a Peer</div>';
    for (const peer of peers) {
        const presentation = peerPresentation(peer, null);
        html +=
            '<button type="button" class="as-detail-sub-row multi-peer-row" data-peer-id="' +
            escapeHtml(peer.id) +
            '" style="width:100%;cursor:pointer;padding:4px 0;' +
    ...
    return (
        html +
        '</div></div><div class="peer-popup-footer">' +
...
function create(options) {
    ...
    let groupLocations = null;
    /** @param {import('../types').Peer} peer */
    const locationKey = (peer) =>
        peer.lat != null && peer.lon != null && (peer.network === 'ipv4' || peer.network === 'ipv6')
            ? `${peer.lat},${peer.lon}`
            : `peer:${peer.id}`;
    function refreshGroup() {
        const locations = groupLocations;
        if (locations)
            groupPeerIds = options
                .getPeers()
                .filter((peer) => locations.has(locationKey(peer)))
                .map((peer) => peer.id);
    }
    /** @type {string | null} */
    let source = null;
    ...
    function mount(html, ariaLabel, borderColor) {
        ...
        query('.peer-popup-close', popup)?.focus({ preventScroll: true });
        return popup;
    }
    ...
    function openGroup(peerIds) {
        ...
        selectedPeerId = null;
        source = 'map-group';
        const peers = groupPeerIds.map(peerFor).filter((peer) => peer !== undefined);
        groupLocations = new Set(peers.map(locationKey));
    ...
    function previewPeer(peer) {
        ...
        const presentation = peerPresentation(
            peer,
            options.privateNetwork ? null : options.getProviderColor?.(distributionData.parseAsNumber(peer.as))
        );
        const name = required('.peer-popup-name', popup);
        if (!name) return;
        ...
        required('.peer-popup-addr', popup).textContent = peer.addr || '';
        required('.peer-popup-meta', popup).textContent = presentation.networkLabel + ' \u00b7 ' + presentation.directionLabel;
        required('.peer-popup-circle', popup).style.background = presentation.networkColor;
    ...
    function update() {
        ...
        if (selectedPeerId !== null) {
            ...
            if (!groupPeerIds.length) {
                options.onRequestClose();
                return false;
            }
            html = renderPeerGroup(groupPeerIds.map(peerFor).filter((peer) => peer !== undefined));
            required('.peer-popup-name', popup).textContent = groupPeerIds.length + ' Peers at This Location';
        }
        // Keep the popup shell, header, focus, drag handlers, geometry and scroll.
        const template = document.createElement('template');
        template.innerHTML = html;
        const content = required('.peer-popup-scroll', template.content);
        const scroll = required('.peer-popup-scroll', popup);
        if (scroll.innerHTML !== content.innerHTML) {
            const top = scroll.scrollTop;
            const focusedId = document.activeElement instanceof HTMLElement ? document.activeElement.dataset.peerId : undefined;
            scroll.innerHTML = content.innerHTML;
            scroll.scrollTop = top;
            if (focusedId) query(`[data-peer-id="${Number(focusedId)}"]`, scroll)?.focus({ preventScroll: true });
        }
        return true;
    }
    ...
    return Object.freeze({
        ...
        previewPeer,
        restorePreview,
        isOpen: () => !!popup || openTimer !== null,
        getSelectedPeerId: () => selectedPeerId,
    });
}
...
function bindPointerInteractions(popup) {
    ...
    if (header) {
        ...
        header.addEventListener('mousedown', (event) => {
            if (event.target instanceof Element && event.target.closest('button, a, .peer-popup-back')) return;
    ...
    if (handle) {
        handle.addEventListener('mousedown', (event) => {
            ...
            /** @param {MouseEvent} moveEvent */
            const move = (moveEvent) => {
                popup.style.width = Math.max(260, rectangle.width + moveEvent.clientX - startX) + 'px';
                popup.style.maxHeight = 'none';
                popup.style.height = Math.max(200, rectangle.height + moveEvent.clientY - startY) + 'px';
            };
            const stop = () => {
                document.removeEventListener('mousemove', move);
                document.removeEventListener('mouseup', stop);
...
export { create };
export { peerDetailRow };
export { renderServiceFlagList };
export { renderPeerDetails };
export { renderPeerGroup };
