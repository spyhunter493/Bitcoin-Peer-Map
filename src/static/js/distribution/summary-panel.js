...
/**
 * @param {import('../types').SummaryPanelOptions} options
 */
function create(options) {
     ...
     * @param {string} label
     *
     * @param {import('../types').Peer[]} matchedPeers
     */
            ...
            for (var ai = 0; ai < abbrs.length; ai++) {
                ...
                for (var fk in options.serviceFlags) {
                    if (options.serviceFlags.hasOwnProperty(fk) && options.serviceFlags[fk].abbr === abbrs[ai]) {
                        html +=
                            '<div class="as-sub-tt-flag">' +
                            BPMModal.escapeHtml(abbrs[ai]) +
                            ' = ' +
                            BPMModal.escapeHtml(serviceFlagDescription(options.serviceFlags[fk])) +
                            '</div>';
                        found = true;
                        break;
                    }
                }
                if (!found) html += '<div class="as-sub-tt-flag">' + BPMModal.escapeHtml(abbrs[ai]) + '</div>';
            }
            html += '</div>';
        }
    ...
    /**
     * @param {import('../types').DistributionGroup} group
     */
     ...
     * @param {unknown} value
     *
     * @param {import('../types').SummaryCategory} catData
     */
    function summaryInteractiveRow(label, value, catData) {
        var providersJson = BPMModal.escapeHtml(
            JSON.stringify(
                catData.providers.map(function (prov) {
                    return {
                        a: prov.asNumber,
                        n: prov.name,
                        c: prov.color,
    ...
    function buildProviderListHtml(providers, catLabel, navAsNum) {
        ...
        if (catLabel) {
            ...
            for (var ai = 0; ai < abbrs.length; ai++) {
                for (var fk in options.serviceFlags) {
                    if (options.serviceFlags.hasOwnProperty(fk) && options.serviceFlags[fk].abbr === abbrs[ai]) {
                        html +=
                            '<div class="as-sub-tt-flag" style="font-size:10px; color:var(--text-secondary)">' +
                            BPMModal.escapeHtml(abbrs[ai]) +
                            ' = ' +
                            BPMModal.escapeHtml(serviceFlagDescription(options.serviceFlags[fk])) +
                            '</div>';
        ...
        for (var i = 0; i < providers.length; i++) {
            ...
            var peerIdsJson = BPMModal.escapeHtml(JSON.stringify(prov.peerIds || prov.pi));
            html +=
                '<div class="as-sub-tt-peer as-provider-row" data-as="' +
            ...
            html +=
                '<span class="as-sub-tt-id as-provider-click" style="cursor:pointer">' + escapeHtml(prov.asNumber || prov.a) + '</span>';
    ...
    function buildPingPeerListHtml(peers) {
        ...
        for (var pi = 0; pi < peers.length; pi++) {
            ...
            html +=
                ...
                escapeHtml((Object.hasOwn(options.connectionTypeLabels, ct) ? options.connectionTypeLabels[ct] : null) || ct) +
                '</span>';
            html += '</div>';
        }
        html += '</div>';
    ...
    /**
     * @param {ReturnType<typeof import('./data.js').computeSummaryData> & {quality: ReturnType<typeof import('./donut.js').getQuality>}} data
     */
    function renderProvider(data) {
        if (!elements.panel) return;

        // --- Header ---
        var asnEl = query('.as-detail-asn', elements.panel);
        var orgEl = query('.as-detail-org', elements.panel);
        var metaEl = query('.as-detail-meta', elements.panel);
        var barFill = query('.as-detail-bar-fill', elements.panel);
        var pctEl = query('.as-detail-pct', elements.panel);
        var riskEl = query('.as-detail-risk', elements.panel);

        if (asnEl) {
            asnEl.innerHTML =
                '<span style="color:var(--logo-primary, #4a90d9)">PEER ISP</span><br><span style="color:var(--logo-accent, #7ec8e3)">DISTRIBUTION</span> <span style="color:var(--logo-primary, #4a90d9)">SUMMARY</span>';
        ...
        if (barFill) {
            ...
            barFill.style.background =
                data.score >= 8 ? 'var(--ok)' : data.score >= 6 ? 'var(--ok-bright)' : data.score >= 4 ? 'var(--warn)' : 'var(--err)';
        }
        if (pctEl) {
        ...
        // --- Body ---
        var bodyEl = query('.as-detail-body', elements.panel);
        if (!bodyEl) return;
        ...
        html +=
            '<div class="modal-section-title" title="Inbound and outbound peer connections grouped by AS provider. Click provider name to view its panel, click IN/OUT to see peer lists.">Connections by Provider</div>';
        for (var gi = 0; gi < data.connectionGrid.length; gi++) {
            var gItem = data.connectionGrid[gi];
            var totalJson = BPMModal.escapeHtml(JSON.stringify(gItem.totalPeerIds));
            var inJson = BPMModal.escapeHtml(JSON.stringify(gItem.inPeerIds));
            var outJson = BPMModal.escapeHtml(JSON.stringify(gItem.outPeerIds));
            var outSubJson = BPMModal.escapeHtml(JSON.stringify(gItem.outSubtypes));

            if (gItem.isOthers && gItem._othersGroups) {
                // Others row — 3-level: click shows provider list, then provider → peer list
                var othersProvJson = BPMModal.escapeHtml(
                    JSON.stringify(
                        gItem._othersGroups.map(function (g) {
                            return {
                                a: g.asNumber,
                                n: g.asShort || g.asName || g.asNumber,
                                c: g.color || actions.getColorForAsNum(g.asNumber),
                                pc: g.peerCount,
                                pi: g.peerIds,
                            };
                        })
                ...
                html +=
                    '<span class="as-detail-sub-label"><span class="as-grid-dot" style="background:' +
                    gItem.color +
        ...
        // ── Section 3: Networks ──
        html +=
            '<div class="modal-section-title" title="Peer connections grouped by network protocol. IPv4/IPv6 are clearnet, Tor/I2P/CJDNS are anonymous overlay networks.">Networks</div>';
        ...
        // ── Section 4: Hosting ──
        html +=
            '<div class="modal-section-title" title="Peer connections grouped by hosting type. Cloud/Hosting = datacenter, Residential = home ISP, Proxy/VPN = anonymizing relay, Mobile = cellular.">Hosting</div>';
        ...
        // ── Section 5: Countries ──
        html +=
            '<div class="modal-section-title" title="Geographic distribution of peers by country, with provider count showing how many distinct AS providers operate in each country.">Countries</div>';
        ...
        // ── Section 6: Software ──
        html +=
            '<div class="modal-section-title" title="Bitcoin Core client versions running on your peers, grouped by user agent string. Multiple versions is healthy for network resilience.">Software</div>';
        ...
        // ── Section 7: Services ──
        html +=
            '<div class="modal-section-title" title="Service flag combinations advertised by peers. N=Full Chain, W=SegWit, NL=Pruned, P=BIP324 v2, CF=Compact Filters, B=Bloom.">Services</div>';
    ...
    /**
     * @param {ReturnType<typeof import('./data.js').computeCountrySummaryData> & {quality: ReturnType<typeof import('./donut.js').getQuality>}} data
     */
    function renderCountry(data) {
        ...
        if (asnEl) {
            asnEl.innerHTML =
                '<span style="color:var(--logo-primary, #4a90d9)">PEER COUNTRY</span><br><span style="color:var(--logo-accent, #7ec8e3)">DISTRIBUTION</span> <span style="color:var(--logo-primary, #4a90d9)">SUMMARY</span>';
            asnEl.classList.add('as-summary-title');
        }
        if (orgEl) {
        ...
        if (data.topCountry) {
            var topLabel =
                (data.topCountry.countryCode || data.topCountry.asShort || '') +
                '  ' +
                (data.topCountry.countryName || data.topCountry.asName);
            html += row('Top Country', topLabel + ' (' + data.topCountry.peerCount + ')');
        }

        html +=
            '<div class="modal-section-title" title="Click a country to inspect the providers, connection types, software, and services behind that slice.">Countries &amp; Territories</div>';
