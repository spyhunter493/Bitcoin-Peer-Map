...
/**
 * @param {import('../types').EntityPanelOptions} options
 */
function render(options) {
    ...
    var asnEl = query('.as-detail-asn', panelEl);
    var orgEl = query('.as-detail-org', panelEl);
    var metaEl = query('.as-detail-meta', panelEl);
    var barFill = query('.as-detail-bar-fill', panelEl);
    var pctEl = query('.as-detail-pct', panelEl);
    var riskEl = query('.as-detail-risk', panelEl);
    ...
    // Build body
    var bodyEl = query('.as-detail-body', panelEl);
    if (!bodyEl) return;
    ...
    if (seg.isOthers) {
        ...
        /** @type {Record<string, {count: number; peers: import('../types').Peer[]}>} */
        ...
        for (var ock = 0; ock < otherConnKeys.length; ock++) {
            ...
            var octPeerIds = otherConnMap[octKey].peers.map(function (p) {
                return p.id;
            });
        ...
        if (fullGroup.connTypesList && fullGroup.connTypesList.length > 0) {
            for (var cti = 0; cti < fullGroup.connTypesList.length; cti++) {
                ...
                var ctPeerIds = ctItem.peers.map(function (p) {
                    return p.id;
                });
        ...
        if (fullGroup.versions && fullGroup.versions.length > 0) {
            ...
            for (var vi = 0; vi < fullGroup.versions.length; vi++) {
                var vPeerIds = fullGroup.versions[vi].peers.map(function (p) {
                    return p.id;
                });
                html += view.interactiveRow(
                    fullGroup.versions[vi].subver,
                    fullGroup.versions[vi].count + ' peer' + (fullGroup.versions[vi].count !== 1 ? 's' : ''),
                    vPeerIds,
                    'software'
                );
            }
        ...
        if (fullGroup.countries && fullGroup.countries.length > 0) {
            ...
            for (var ci = 0; ci < fullGroup.countries.length; ci++) {
                var cPeerIds = fullGroup.countries[ci].peers.map(function (p) {
                    return p.id;
                });
                html += view.interactiveRow(
                    fullGroup.countries[ci].code + '  ' + fullGroup.countries[ci].name,
                    fullGroup.countries[ci].count,
                    cPeerIds,
                    'country'
                );
            }
        ...
        if (fullGroup.servicesCombos && fullGroup.servicesCombos.length > 0) {
            ...
            for (var si = 0; si < fullGroup.servicesCombos.length; si++) {
                var sPeerIds = fullGroup.servicesCombos[si].peers.map(function (p) {
                    return p.id;
                });
                html += view.interactiveRow(
                    fullGroup.servicesCombos[si].abbrev,
                    fullGroup.servicesCombos[si].count + ' peer' + (fullGroup.servicesCombos[si].count !== 1 ? 's' : ''),
                    sPeerIds,
                    'services'
                );
            }
