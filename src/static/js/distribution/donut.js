...
/**
 * @param {Omit<import('../types').DonutConfig, 'duration' | 'maxSegments'> & {segments: import('../types').DistributionSegment[]; totalPeers: number; selectedProvider: string | null; animation?: Omit<import('../types').DonutAnimation, 'startedAt'>}} options
 */
function buildDonutSvg(options) {
    ...
    html +=
        ...
        '" r="' +
        (radius - width / 2) +
        '" fill="none" stroke="rgba(88,166,255,0.04)" ' +
        'stroke-width="' +
        width +
        '" />';
    html +=
        ...
        '" r="' +
        (radius + 3) +
        '" fill="none" stroke="rgba(88,166,255,0.08)" stroke-width="1" />';
    html +=
        '<circle cx="' +
        center +
        '" cy="' +
        ...
        '" r="' +
        (innerRadius - 3) +
        '" fill="none" stroke="rgba(88,166,255,0.06)" ' +
        'stroke-width="0.5" />';
    ...
    if (segments.length === 1) {
        ...
        const singleLabel = (segment.asShort || segment.asName || segment.asNumber) + ', ' + segment.peerCount + ' peers';
        return (
            html +
            '<circle cx="' +
            center +
            ...
            (radius - width / 2) +
            '" fill="none" stroke="' +
            escapeHtml(segment.color) +
            '" stroke-width="' +
            width +
            '" class="as-donut-segment" data-as="' +
            escapeHtml(segment.asNumber) +
            '" filter="url(#donut-shadow)" tabindex="0" ' +
            'role="button" aria-label="' +
            escapeHtml(singleLabel) +
            '" />'
        );
    }

    const available = 2 * Math.PI - gap * segments.length;
    const normalSweeps = segments.map((segment) => (totalPeers > 0 ? (segment.peerCount / totalPeers) * available : 0));
    ...
    if (animating) {
        ...
        if (targetIndex >= 0) {
            ...
            for (let index = 0; index < segments.length; index += 1) {
                expandedSweeps[index] =
                    index === targetIndex
                        ? expandedSweep
                        : (otherTotal > 0 ? segments[index].peerCount / otherTotal : 1 / (segments.length - 1)) * remaining;
            }
        }
        let progress = animation.state === 'reverting' ? 1 - animation.progress : animation.progress;
        progress = eased(progress);
        sweeps = normalSweeps.map((sweep, index) => sweep + (expandedSweeps[index] - sweep) * progress);
    }
    ...
    if (animating) {
        animationProgress = animation.state === 'reverting' ? 1 - animation.progress : animation.progress;
    ...
    segments.forEach((segment, index) => {
        ...
        const path = describeArc(center, center, outerRadius, outerRadius - segmentWidth, angles[index].start, angles[index].end);
        ...
        const segmentLabel = (segment.asShort || segment.asName || segment.asNumber) + ', ' + segment.peerCount + ' peers';
        html +=
            '<path d="' +
            path +
            '" fill="' +
    ...
    return (
        ...
        '" r="' +
        (radius - width / 2) +
        '" fill="none" stroke="url(#donut-highlight)" ' +
        'stroke-width="' +
        width +
        '" pointer-events="none" />'
    );
...
function legendItem(segment, className, totalPeers, color) {
    ...
    return (
        ...
        '">' +
        '<span class="as-legend-dot" style="background:' +
        escapeHtml(color || segment.color) +
        '"></span><span class="as-legend-name" title="' +
        escapeHtml(displayName) +
        '">' +
        escapeHtml(shortName) +
        '</span><span class="as-legend-count">' +
...
/**
 * @param {import('../types').DonutView & {focusProvider?: string | null; selectedProvider?: string | null; maxSegments: number; getColor(provider: string): string}} options
 */
...
function buildInsightHtml(type, data) {
    ...
    return (
        '<div class="as-insight-rect-inner"><div class="as-insight-rect-badge">' +
        ...
        presentation.icon +
        '</div><div class="as-insight-rect-title">' +
        escapeHtml(presentation.title) +
        '</div>' +
        (presentation.rank ? '<div class="as-insight-rect-rank" style="color:#d4a017">' + escapeHtml(presentation.rank) + '</div>' : '') +
        ...
        '">' +
        escapeHtml(presentation.providerName) +
        '</div><div class="as-insight-rect-meta">' +
        escapeHtml(presentation.metadata) +
        '</div>' +
        (presentation.statistic
            ? '<div class="as-insight-rect-stat" style="color:' +
              escapeHtml(presentation.color) +
              '">' +
              escapeHtml(presentation.statistic) +
              '</div>'
            : '') +
...
/**
 * @param {import('../types').DonutOptions} options
 */
function create(options) {
    const state = options.state;
    const motionQuery = globalThis.matchMedia ? globalThis.matchMedia('(prefers-reduced-motion: reduce)') : null;
    ...
    /** @type {import('../types').DonutElements} */
    let elements = {};
    ...
    /** @type {import('../types').DonutAnimation} */
    let animation = { state: 'idle', target: null, progress: 0, startedAt: 0 };

    /**
     * @param {import('../types').DonutElements} nextElements
     */
    ...
    function bindSegmentEvents(root) {
        if (!root) return;
        queryAll('.as-donut-segment, .as-legend-item', root).forEach((element) => {
    ...
    function renderDonut() {
        ...
        elements.svg.innerHTML = buildDonutSvg(
    ...
    function animationStep(now) {
        ...
        if (animation.progress < 1) {
            animationFrame = globalThis.requestAnimationFrame(animationStep);
            return;
        }
        animationFrame = null;
    ...
    function centerParts() {
        ...
        return {
            distribution: query('.as-score-distribution', elements.center),
            heading: query('.as-score-heading', elements.center),
            value: query('.as-score-value', elements.center),
            quality: query('.as-score-quality', elements.center),
            label: query('.as-score-label', elements.center),
        };
    }
    ...
    /** @param {import('../types').DistributionSegment} segment
     *
     * @param {import('../types').ProviderCenterOptions} [settings]
     */
    function setProviderCenter(segment, settings) {
        ...
        if (parts.heading) {
            parts.heading.textContent = opts.legendHover && !segment.isOthers ? opts.entityKind : '';
        ...
        const name = segment.isOthers ? 'Others' : segment.asShort || segment.asName || segment.asNumber;
        let displayName = opts.focused || opts.legendHover ? formatNameForDonut(name) : name;
        if (!opts.focused && !opts.legendHover && displayName.length > 14) {
            displayName = displayName.substring(0, 13) + '…';
        }
        if (parts.value) {
            parts.value.textContent = displayName;
            parts.value.className =
                'as-score-value ' +
                (opts.legendHover ? 'as-legend-hover-provider' : opts.focused ? 'as-focused-provider' : 'as-selected-mode');
            parts.value.style.color = segment.color;
            parts.value.title =
                (segment.asName || segment.asNumber) + '\n' + segment.peerCount + ' peers (' + segment.percentage.toFixed(1) + '%)';
        }
        if (parts.quality) {
            parts.quality.textContent =
                segment.asNumber === 'Others'
                    ? opts.focused || opts.legendHover
                        ? segment.asName || ''
                        : ''
                    : opts.countryLens
                      ? segment.countryCode || ''
        ...
        if (parts.label) {
            parts.label.textContent = segment.peerCount + ' PEER' + (segment.peerCount !== 1 ? 'S' : '');
        ...
        const backLink = elements.center && query('.as-others-back-link', elements.center);
     ...
     * @param {number} totalPeers
     *
     * @param {{countryLens?: boolean; tooltip?: string}} [settings]
     */
    function renderScoreCenter(score, totalPeers, settings) {
        ...
        parts.value.classList.remove('as-score-excellent', 'as-score-good', 'as-score-moderate', 'as-score-poor', 'as-score-critical');
    ...
    function showInsight(type, data) {
        ...
        if (globalThis.document && globalThis.document.body) {
            globalThis.document.body.classList.add('insight-rect-active');
        }
        const closeButton = query('.as-insight-rect-close', elements.insight);
    ...
    function updateInsightPeer(peer, color, type) {
        ...
        const provider = query('.as-insight-rect-provider', elements.insight);
        const metadata = query('.as-insight-rect-meta', elements.insight);
        const statistic = query('.as-insight-rect-stat', elements.insight);
        const rank = query('.as-insight-rect-rank', elements.insight);
        ...
        if (statistic) {
            if (type === 'fastest') {
                ...
                const connectedSeconds = peer.conntime ? Math.floor(Date.now() / 1000) - peer.conntime : 0;
    ...
    function hideInsight() {
        ...
        if (globalThis.document && globalThis.document.body) {
            globalThis.document.body.classList.remove('insight-rect-active');
        }
    ...
    function updateLoading(pendingCount, isLoading) {
        ...
        if (isLoading) {
            elements.loading.textContent = 'Locating ' + pendingCount + ' peer' + (pendingCount !== 1 ? 's' : '') + '…';
    ...
    function getLegendDotPosition(provider) {
        ...
        const items = queryAll('.as-legend-item', elements.legend);
        for (const item of items) {
            if (item.dataset.as !== provider) continue;
            const dot = query('.as-legend-dot', item);
            if (!dot) return null;
    ...
    function getInsightOrigin() {
        ...
        const dot = query('.as-insight-rect-origin', elements.insight);
        if (!dot) return null;
        const rect = dot.getBoundingClientRect();
        if (rect.width === 0 && rect.height === 0) return null;
        return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    ...
    function highlightLegend(provider) {
        if (!elements.legend) return;
        queryAll('.as-legend-item', elements.legend).forEach((item) => {
...
export { buildInsightHtml };
export { buildLegendHtml };
export { buildScoreTooltip };
export { create };
export { describeArc };
export { formatNameForDonut };
export { getQuality };
export { insightPresentation };
