/* Pure peer-table selection and sorting over the shared API peer schema. */
(function (global) {
    'use strict';
    ...
    function filterPeers(peers, filters) {
        return peers.filter(peer => {
            if (filters.privateMode) {
                return ['onion', 'i2p', 'cjdns'].includes(peer.network) &&
                    (!filters.privateNetwork || peer.network === filters.privateNetwork);
            }
            return filters.passesNetwork(peer.network || 'ipv4') &&
    ...
    function sortPeers(peers, column, ascending) {
        ...
        return sorted.sort((a, b) => {
            ...
            // Older connection timestamps represent longer durations.
                ...
                comparison = typeof left === 'number' && typeof right === 'number'
    ...
    global.BPMPeerTableModel = Object.freeze({ filterPeers, sortPeers });
})(window);
