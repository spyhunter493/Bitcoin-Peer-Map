...
function networkDefinition(networkKey) {
    return networkKey === 'ipv4' ? NETWORKS.ipv4 : NETWORKS.ipv6;
}
...
/**
 * @param {import('../types').SummaryCategory[]} categories
 */
function renderCategoryRows(categories) {
    return categories
        .map((category) => renderInteractiveRow(category.label, category.peerCount + 'p / ' + category.providerCount + 'prov', category))
        .join('');
}

/**
 * @param {ReturnType<typeof computeNetworkPanelData>} data
 */
function renderNetworkPanelBody(data) {
    ...
    html +=
        '<div class="modal-section-title" title="' +
        label +
        ' peer connections grouped by AS provider">' +
        label +
    ...
    html += '<div class="modal-section-title" title="Geographic distribution of ' + label + ' peers by country">Countries</div>';
    html += renderCategoryRows(data.countries);

    html += '<div class="modal-section-title" title="Bitcoin Core client versions on ' + label + ' peers">Software</div>';
    html += renderCategoryRows(data.software);

    html += '<div class="modal-section-title" title="Service flags on ' + label + ' peers">Services</div>';
...
function setPeerCount(panel, peerCount) {
    const organization = panel && query('.as-detail-org', panel);
    if (organization) {
        organization.textContent = peerCount + ' peer' + (peerCount !== 1 ? 's' : '') + ' connected';
    }
}
...
function updateHeader(panel, data) {
    const asn = query('.as-detail-asn', panel);
    const metadata = query('.as-detail-meta', panel);
    const barFill = query('.as-detail-bar-fill', panel);
    const percentage = query('.as-detail-pct', panel);
    const risk = query('.as-detail-risk', panel);
...
/**
 * @param {{peers: import('../types').Peer[]; segments: import('../types').DistributionSegment[]; networkKey: string; panelElement: HTMLElement | null; isRefresh?: boolean}} options
 */
function render(options) {
    const data = computeNetworkPanelData(options.peers, options.networkKey, options.segments);
    const panel = options.panelElement;
    if (!panel) return { data, bodyElement: null };

    updateHeader(panel, data);
    const body = query('.as-detail-body', panel);
...
export { peersForNetwork };
export { computeNetworkPanelData };
export { renderStatRow };
export { renderInteractiveRow };
export { renderNetworkPanelBody };
export { render };
